...
void * Reconnect(void *args) {
    modbus_t *ctx;
    while (1) {
        if(!work) pthread_exit(0);
        //        printf("Start reconect...\n");
        if (sleep(RECONECT_TIME) != 0) pthread_exit(0);
        //        printf("Continue reconect...\n");
        //        continue;
        updateDataSaver();
        if (driversRun)WakeUpDriver();
...
ContextServer * master_init(ModbusDevice *mdev) {
    if (mdev == NULL) return NULL;

    if (mdev->typeDevice != 1) return NULL; // не master устройство
...
ContextServer * slsrv_init(ModbusDevice * mdev) {
    ...
    if (mdev->typeDevice != 0) return NULL; // не slave устройство
    ContextServer *cnt = malloc(sizeof (ContextServer));
    if (cnt == NULL) return NULL;
    cnt->port = mdev->port;
...
int master_step(ModbusDevice * md) {
    ...
    if (cnt->stage == 0) {
        if (time_after(&cnt->tstart) < cnt->step) return 0;
    }
    if (cnt->stage == 0) {
